...
from fast_api_builder.crud.graphql_schema_gen import generate_schemas
from fast_api_builder.utils.file import get_file_content, get_package_file, write_to_file
from fast_api_builder.utils.str_helpers import remove_between, to_snake_case


common_template = 'fast_api_builder.common.templates'
...
def generate_schema(module, model: str, with_controller: bool, create_multiple: bool, with_attachment: bool, with_transition: bool):
    ...
    for model_name in models:
        ...
        if with_attachment:
            real_api_template = real_api_template.replace(f"from fast_api_builder.attach.response import {model_name}AttachmentResponse", '')\
                                                .replace(f"from fast_api_builder.attach.request import {model_name}Attachment", '')
        else:
            # Remove Attachments Queries
            real_api_template = remove_between(real_api_template, 
                                    "'''ATTACHMENT_QUERIES'''", 
        ...
        # Create Attachment Quaries and Mutations
        if with_transition:
            real_api_template = real_api_template.replace(f"from fast_api_builder.workflow.response import {model_name}EvaluationStatusResponse", '')
        else:
            # Remove Attachments Queries
            real_api_template = remove_between(real_api_template, 
                                    "'''EVALUATION_QUERIES'''", 
            ...
            # Also remove imports
            real_api_template = real_api_template.replace(f"from fast_api_builder.workflow.response import {model_name}EvaluationStatusResponse", '')\
                                                .replace('from fast_api_builder.workflow.request import EvaluationStatus', '')
        ...
        if with_controller:
            ...
            real_api_template = real_api_template.replace('fast_api_builder.crud.gql_controller', f"modules.{module}.controllers.{to_snake_case(model_name)}")\
        ...
        if with_attachment:
            if not has_class(f"{schemas_path}/request.py", f"{model_name}Attachment"):
                ...
                if not module_request.__contains__("from fast_api_builder.attach.request import AttachmentUpload"):
        ...
        if with_attachment:
            if not has_class(f"{schemas_path}/response.py", f"{model_name}AttachmentResponse"):
                ...
                if not module_response.__contains__("from fast_api_builder.attach.response import AttachmentResponse"):
                    module_response = module_response.split("'''List of Types'''")[0] \
                        + "from fast_api_builder.attach.response import AttachmentResponse"\
                        + "\n'''List of Types'''"\
        ...
        if with_transition:
            if not has_class(f"{schemas_path}/response.py", f"{model_name}EvaluationStatusResponse"):
                ...
                if not module_response.__contains__("from fast_api_builder.workflow.response import EvaluationStatusResponse"):
                    module_response = module_response.split("'''List of Types'''")[0] \
